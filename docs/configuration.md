...
| `UAF_RESOLVED_TTL_SECONDS` | `"300"` | Short TTL for resolved entries whose RESTAction carries a `UserAuthorizationFilter`, capping how long an authorization change can be served stale (#118 (d)); `0` disables. 1.12.3 also declines the cache write for those cells, so this is belt-and-braces until the per-user RBAC-generation cache key lands in 1.13.0. |
