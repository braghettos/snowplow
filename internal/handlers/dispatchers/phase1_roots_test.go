...
func TestFrontendConfig_RootsDecodeFromConfigNotLiterals(t *testing.T) {
	...
	initRef, ok := parseCallPathToObjectRef(cfg.API.Init)
	if !ok {
	...
	rlRef, ok := parseCallPathToObjectRef(cfg.API.RoutesLoader)
	if !ok {
...
func TestFrontendConfig_MissingEntryPointTolerated(t *testing.T) {
	...
	if _, ok := parseCallPathToObjectRef(cfg.API.RoutesLoader); !ok {
...
func TestFrontendConfig_NonCallURLRejected(t *testing.T) {
	for _, bad := range []string{
		...
		if _, ok := parseCallPathToObjectRef(bad); ok {
